...
try {
        const editId = document.getElementById('opportunityForm').getAttribute('data-edit-id');
...
async function loadOpportunities() {
    try {
        ...
        if (opportunities.length === 0) {
            grid.innerHTML = '<p style="color:var(--qf-text);padding:20px;">No opportunities created yet.</p>';
            return;
        }

        opportunities.forEach(opp => addOpportunityCard(opp));
...
// Add a single opportunity card to the grid
    ...
    card.innerHTML = `
        ...
        <div class="opportunity-footer">
            ...
            <div style="display:flex;gap:8px;">
                <button class="view-course-btn" style="width:auto;padding:8px 16px;" onclick="viewOpportunityDetails(${opp.id})">View Details</button>
                <button class="view-course-btn" style="width:auto;padding:8px 16px;background:#1a5c38;" onclick="editOpportunity(${opp.id})">Edit</button>
                <button class="view-course-btn" style="width:auto;padding:8px 16px;background:#c0392b;" onclick="deleteOpportunity(${opp.id})">Delete</button>
            </div>
        </div>
    `;
...
// Edit opportunity — pre-fill form and resubmit as PUT
